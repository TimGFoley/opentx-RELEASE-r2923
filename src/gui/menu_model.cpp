...
#endif
          if (active) ld->min = 100 + checkIncDecModel(event, ld->min-100, -limit, 0);
          break;
...
#endif
          if (active) ld->max = -100 + checkIncDecModelZero(event, ld->max+100, limit);
          break;
...
#if defined(PCBTARANIS)
//  #define USRDATA_LINES (uint8_t)-1, 0, // T.Foley
  #define USRDATA_LINES (uint8_t)-1, 0, 0, 0, // T.Foley
  ...
  for (uint8_t i=0; i<LCD_LINES-1; i++) {
    ...
    switch(k) {
      ...
      case ITEM_TELEMETRY_USR_BLADES:
        ...
        //lcd_outdezAtt(TELEM_COL2+FWNUM, y, 2+g_model.frsky.blades, attr); // T.Foley
        //if (attr) CHECK_INCDEC_MODELVAR_ZERO(event, g_model.frsky.blades, 3);// T.Foley
